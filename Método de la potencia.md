...
pero notemos que si $k \to +\infty$, entonces $r^{(k)} := \alpha_1v_1 + \sum_{i = 2}^n \alpha_i \left ( \frac{\lambda_i}{\lambda_1}^k v_i \right ) \to 0$, dado que $|\lambda_1| > |\lambda_i|$, $\forall 2 \le i \le n$. Luego para $k$ grande, $y^{(k)} \approx \lambda_1^k \alpha_1 v_1$. Es decir, $y^{(k)}$ es esencialmente un vector propio de $A$.
